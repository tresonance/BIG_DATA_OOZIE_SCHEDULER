...
#
#
#      spark-submit --queue default --master local --num-executors 10 --executor-memory 12g --executor-cores 4 --driver-memory 12g create_dataframe.py
#
# ####################################################################################
import sys
import os 
...
sparkSession = lib.init_spark( app_name = "create_dataframe.py" )
...
def main():
    ...
    try:
        data = [
            ...
            ('Michael', 'M', '1972-09-23'),
             ('Ibrahima', 'M', '2015-08-02'),
            ('Soro', 'M', '2011-09-23'),
            ('Brice', 'M', '2018-03-23'),
            ('Salimata', 'F', '1988-05-23'),
        ...
        schema = StructType([
            ...
            StructField('weeding', StringType(), True)
        ])
    ...
    lib.write_data_info("###############################################\n\
    # [ STEP 2]\n\
    # VERIFICATION ECRITURE DATAFRAME VERS "+ parquet_path +"\n\
    #\n\
    #####################################################################", logger)
    try:
        df_check = sparkSession.read.parquet(parquet_path)
        lib.write_data_info(lib.getShowString(df_check), logger)
    except Exception as ex:
        print(ex)
        lib.error_exit(logger, logs_file, hdfs_logs_file, ex, additive_infos="Unexpected error while checking")
        exit()
